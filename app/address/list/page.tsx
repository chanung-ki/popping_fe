...
const AdressListPage: React.FC = () => {
  ...
  const AddressDelete = async (id: number, deleteAble: boolean) => {
    if (deleteAble) {
      ...
      try {
        const response = await axiosInstance.delete(`/api/user/address?id=${id}`)

        if (response.status === 202) {
          alert('배송지가 삭제되었습니다.')
          const updatedAddressData = addressData?.filter((address) => address.id !== id);
  ...
  const AddressDefaultUpdate = async (id: number, updateAble: boolean) => {
    if (updateAble) {
      ...
      try {
        const response = await axiosInstance.patch(`/api/user/address?id=${id}`)
        ...
        if (response.status === 202) {
          ...
          const sortedData = updatedAddressData?.sort((a, b) => (b.default ? 1 : -1));
          setAddressData(sortedData);
        }
      } catch (error: any) {
      }
  ...
  const GoToEdit = (id: number) => {
    router.push(`/address/edit/?id=${id}&redirect=${encodeURIComponent(window.location.href)}`)
  }
  ...
  const AddressAdd = (addAble: boolean) => {
    if (addAble) {
      router.push(`/address/add?redirect=${encodeURIComponent(window.location.href)}`)
    }
    else {
      alert(`배송지를 추가할 수 있는 최대치입니다.\n현재 : ${addressData.length}개`)
    }
  }
        ...
        <AddressesContainer>
          ...
          {addressData.map((data: UserAddress, index: number) => (
            <AddressBox key={`address-${index}`}>
              ...
              <AddressText>({data.postNumber}) {data.address}, {data.detailAddress}</AddressText>
              <OptionsContainer>
                {!data.default && <SetDefaultText onClick={() => AddressDefaultUpdate(data.id, data.default)}>기본 배송지 선택</SetDefaultText>}
                <OptionText onClick={() => GoToEdit(data.id)}>수정</OptionText>
                <OptionText onClick={() => AddressDelete(data.id, data.default)}>삭제</OptionText>
              </OptionsContainer>
            </AddressBox>
          ))}
        </AddressesContainer>
